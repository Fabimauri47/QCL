...
def multiplicacion(c1,c2):
    pReal = c1[0] * c2[0]
    pImag = c1[0] *c2[1] + c1[1] * c2[0] + c1[1] * c2[1]

    return pReal,pImag

def division(c1,c2):
    pReal = (c1[0] * c2[0] + c1[1]* c2[1]) / ((c1[0]**2) + (c2[0]**2))
    pImag = (c2[0] * c1[0] - c1[1]* c2[1]) / ((c1[0]**2) + (c2[0]**2))

    return pReal, pImag

def modulo(c1,c2):
    mod = ((c1[0]**2) + (c2[0]**2))**0.5
    return mod
...
def cartesiono_A_polares(c1,c2):
    pReal=(((c1[0])**2)+((c2[0])**2))**0.5
    pImag= math.atan(c1[0]/c2[0])
    return pReal,pImag

def polar_A_cartesiano(c1,c2):
    pReal=(c1[0]*math.cos(c2[0]))
    pImag=(c1[0]*math.sin(c2[0]))
    return pReal,pImag

def fase(c1,c2):
    ANGULO=math.atan(c1[0]/c2[0])
    return ANGULO
